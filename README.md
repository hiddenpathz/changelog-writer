...
Это legacy PHP-версия `changeloger`.
Последний релиз этой ветки оставлен для совместимости с Composer/Packagist.

Дальше основная версия развивается как standalone Go CLI без PHP и Composer:

https://github.com/hiddenpathz/changeloger

Перед архивированием этой ветки можно выпустить последний PHP-релиз с текущими исправлениями:

- первая буква описания коммита поднимается в uppercase;
- секции CHANGELOG.md выводятся в регламентном порядке: `Реализовано`, `Изменено`, `Исправлено`, `Удалено`.
